...
However, as noted in \cref{sec:numer-runn-time}, doing so means the algorithm takes a long time to converge.
...
For example, for $4$~states, our transition matrix is
\begin{equation*}
  \begin{bmatrix}
    0.75 & 0.25 & 0    & 0\\
    0    & 0.75 & 0.25 & 0\\
  ...
  \end{bmatrix}
\end{equation*}
% TODO observations of this matrix as we run our algorithm

% TODO emission matrix numerical issues
...
\Cref{fig:state-in-the-middle} shows how these initial observations result in points being assigned to a state that does not correspond to a location in the tail of the robot's path.
\begin{wrapfigure}{o}{0.5\textwidth}
  \centering
...
% how we use viterbi to determine the most likely states given the parameters computer in Baum Welch and the observed angles

\section{Mapping States to Locations}\label{sec:mapp-stat-locat}

% how we use labels to map states to locations
